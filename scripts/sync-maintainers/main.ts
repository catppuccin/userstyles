...
import { getUserstylesData } from "../utils/data.ts";
import {
...
const maintainers = [
  ...new Set(
    Object.values(userstyles).flatMap((style: Userstyle) =>
...
const maintainersTeamMembers = await getUserstylesTeamMembers(octokit, team);

if (
  isDeepStrictEqual(maintainers.toSorted(), maintainersTeamMembers.toSorted())
) {
...
for (const member of toRemove) {
  ...
  console.log(`Removed ${member} from the catppuccin/${team} team.`);
}
console.log(
